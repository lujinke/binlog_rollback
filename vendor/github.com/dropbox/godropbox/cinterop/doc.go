// This software package is designed to help interop between legacy C programs
// and go programs.
// If a primarily C program needs to call a utility function that is only
// available in go, this package can assist with interop. In essence this package
// lets you call golang functions from C if the go functions take in simple byte
// arrays as input.
...
// This allows fixed sized structs to be processed efficicently and it takes
// care of reading the input on struct sized boundaries and will never call the
// bound function, taking a []byte with less than a full workSize elements.
...
// preloading of another command while output is being sent back to the C
// caller.
...
// example_batched.go shows how this can be utilized.
